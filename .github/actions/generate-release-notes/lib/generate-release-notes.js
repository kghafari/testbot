...
export async function manageReleases() {
    ...
    try {
        ...
        if (deploymentStatusEvent.deployment.environment === BETA_ENV) {
            await createRelease(deploymentStatusEvent, BETA_ENV, latestReleaseCommitish, currentDeploymentSha, DRAFT_NAME, true);
        }
        // 5. On prod deploy create a release
            ...
            const releaseName = new Date()
                ...
                .replace(/\.\d+Z$/, '')
                .replace('T', '-');
            await createRelease(deploymentStatusEvent, PROD_ENV, latestReleaseCommitish, currentDeploymentSha, releaseName, false);
            ...
            await createRelease(deploymentStatusEvent, BETA_ENV, latestReleaseResponse.data.target_commitish, lastSuccessfulNonprodDeploymentSha, DRAFT_NAME, true);
        }
    }
    catch (error) {
...
manageReleases();
async function createRelease(deploymentStatusEvent, env, from, to, name, draft = true) {
...
async function getLatestReleaseCommitish(owner, repo, fallbackSha = '') {
    try {
        ...
        const { data: latestReleaseResponse } = await octokit.rest.repos.getLatestRelease({
            owner: owner,
            repo: repo,
        });
        core.info(`found ${latestReleaseResponse.target_commitish.slice(0, 7)}: ${latestReleaseResponse.html_url}`);
    ...
    for (const commit of commits) {
        ...
        try {
            ...
            if (prResponse.length > 0) {
                ...
                core.info(`Found PR for commit ${commitSha.slice(0, 7)} - ${prTitle} by @${prAuthor} in https://github.com/${owner}/${repo}/pull/${prNum}\n`);
